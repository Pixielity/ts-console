...
var Application = class {
  ...
  registerWithCommander(command) {
    ...
    const commanderCommand = new commander.Command(command.getName()).description(command.getDescription()).action(async (...args) => {
      try {
        ...
        console.error(
          chalk__default.default.red(`Error: ${error instanceof Error ? error.message : String(error)}`)
        );
        process.exit(1);
      }
    });
  ...
  async run(argv = process.argv) {
    try {
      ...
      console.error(
        chalk__default.default.red(`Fatal error: ${error instanceof Error ? error.message : String(error)}`)
      );
      process.exit(1);
    }
  }
...
var ConsoleServiceProvider = class {
   ...
   */
  constructor(container2, commandsDir = path__namespace.join(process.cwd(), "src/commands"), stubsDir = path__namespace.join(process.cwd(), "src/stubs/templates")) {
    /**
  ...
  register() {
    ...
    this.bindIfNotBound(tsTypes.IConsoleApplication.$, Application);
    this.bindIfNotBound(tsTypes.ICommandRegistry.$, CommandRegistry);
    this.bindIfNotBound(tsTypes.ICommandScheduler.$, CommandScheduler);
    this.bindIfNotBound(tsTypes.IStubGenerator.$, StubGenerator, new StubGenerator(this.stubsDir));
    this.bindIfNotBound(tsTypes.ITableOutput.$, TableOutput, null, false);
    this.bindIfNotBound(tsTypes.IProgressBar.$, ProgressBar, null, false);
    this.bindIfNotBound(tsTypes.IAsk.$, null, Ask);
    this.bindIfNotBound(tsTypes.ICommandCollector.$, CommandCollector);
   ...
   */
  async boot() {
    const app = this.container.get(tsTypes.IConsoleApplication.$);
...
//# sourceMappingURL=console-service-provider.js.map
//# sourceMappingURL=console-service-provider.js.map