...
import { ICommandRegistry, ICommandCollector, IStubGenerator, ICommandScheduler, ICommand, QuestionType } from '@pixielity/ts-types';
...
var Output = class {
   ...
   */
  error(message) {
    console.error(chalk6.bold.red("ERROR") + ": " + message);
  }
  /**
   ...
   */
  success(message) {
    console.log(chalk6.bold.green("SUCCESS") + ": " + message);
  }
  /**
   ...
   */
  info(message) {
    console.log(chalk6.bold.blue("INFO") + ": " + message);
  }
  /**
   ...
   */
  warning(message) {
    console.log(chalk6.bold.yellow("WARNING") + ": " + message);
  }
  /**
...
var Application = class {
  ...
  constructor(commandRegistry, commandCollector, commands = [], name = "Next.js Console", version = "1.0.0") {
    ...
    this.program = new Command$1().name(this.name).version(this.version).description(`${this.name} - A Laravel/Symfony-inspired console for Next.js`).helpOption("-h, --help", "Display help for command").helpCommand("help [command]", "Display help for command");
  ...
  setCommands(commands) {
    ...
    this.program = new Command$1().name(this.name).version(this.version).description(`${this.name} - A Laravel/Symfony-inspired console for Next.js`).helpOption("-h, --help", "Display help for command").addHelpCommand("help [command]", "Display help for command");
    this.addGlobalOptions();
    this.registerCommands(commands);
    return this;
  }
  ...
  registerWithCommander(command) {
    ...
    const commanderCommand = new Command$1(command.getName()).description(command.getDescription()).action(async (...args) => {
      try {
        ...
        console.error(
          chalk6.red(`Error: ${error instanceof Error ? error.message : String(error)}`)
        );
        process.exit(1);
      }
    });
  ...
  async run(argv = process.argv) {
    try {
      ...
      console.error(
        chalk6.red(`Fatal error: ${error instanceof Error ? error.message : String(error)}`)
      );
      process.exit(1);
    }
  }
...
var ProgressBar = class {
  ...
  constructor(total = 100, format) {
    this.bar = new cliProgress.SingleBar({
      format: (format == null ? void 0 : format.format) || `${chalk6.cyan("{bar}")} {percentage}% | ETA: {eta}s | {value}/{total}`,
  ...
  static createMultiBar() {
    return new cliProgress.MultiBar({
      format: `${chalk6.cyan("{bar}")} {percentage}% | ETA: {eta}s | {value}/{total}`,
      barCompleteChar: "\u2588",
      barIncompleteChar: "\u2591"
    });
  }
...
var DemoCommand = class extends BaseCommand {
  ...
  async demoAsk() {
    ...
    this.line();
    this.line(`Name: ${chalk6.cyan(name)}`);
    this.line(`Age: ${chalk6.cyan(age)}`);
    this.line(`Likes CLI: ${likesCli ? chalk6.green("Yes") : chalk6.red("No")}`);
    this.line(`Favorite color: ${chalk6.cyan(favoriteColor)}`);
    this.line(`Languages: ${languages.map((l) => chalk6.cyan(l)).join(", ")}`);
    this.line();
  }
  /**
...
var GreetCommand = class extends BaseCommand {
  ...
  async execute() {
    try {
      ...
      switch (color) {
        case "red":
          coloredGreeting = chalk6.red(greeting);
          break;
        case "green":
          coloredGreeting = chalk6.green(greeting);
          break;
        case "blue":
          coloredGreeting = chalk6.blue(greeting);
          break;
        case "yellow":
          coloredGreeting = chalk6.yellow(greeting);
          break;
        case "cyan":
          coloredGreeting = chalk6.cyan(greeting);
          break;
        default:
          coloredGreeting = chalk6.green(greeting);
      }
...
var HelpCommand = class extends BaseCommand {
  ...
  async execute() {
    try {
      ...
      const metadata = Reflect.getMetadata(COMMAND_METADATA_KEY, command.constructor) || {};
      this.line(chalk6.bold(`${command.getName()}: ${command.getDescription()}`));
      this.line("");
      if (metadata.aliases && metadata.aliases.length > 0) {
        this.line(chalk6.cyan("Aliases:"));
        metadata.aliases.forEach((alias) => {
          this.line(`  ${chalk6.yellow(alias)}`);
        });
        this.line("");
      }
      if (metadata.shortcuts && metadata.shortcuts.length > 0) {
        this.line(chalk6.cyan("Shortcuts:"));
        metadata.shortcuts.forEach((shortcut) => {
          this.line(`  ${chalk6.magenta(shortcut.flag)}: ${shortcut.description}`);
        });
        this.line("");
      }
      const argumentsMetadata = Reflect.getMetadata(ARGUMENT_METADATA_KEY, command.constructor) || [];
      if (argumentsMetadata.length > 0) {
        this.line(chalk6.cyan("Arguments:"));
        argumentsMetadata.forEach((arg) => {
          this.line(`  ${chalk6.green(arg.name)}: ${arg.description || "No description"}`);
        });
        this.line("");
      }
      const optionsMetadata = Reflect.getMetadata(OPTION_METADATA_KEY, command.constructor) || [];
      if (optionsMetadata.length > 0) {
        this.line(chalk6.cyan("Options:"));
        optionsMetadata.forEach((opt) => {
          this.line(`  ${chalk6.green(opt.flags)}: ${opt.description || "No description"}`);
        });
        this.line("");
      }
      return BaseCommand.SUCCESS;
...
var ListCommand = class extends BaseCommand {
  ...
  async execute() {
    try {
      ...
      this.line(chalk6.bold("Available commands:"));
      this.line(chalk6.dim("Use command name, alias, or shortcut to run a command"));
      this.line("");
      if (commands.length === 0) {
      ...
      for (const [category, groupCommands] of Object.entries(commandGroups)) {
        this.line(chalk6.cyan(`${category.charAt(0).toUpperCase() + category.slice(1)} Commands:`));
        ...
        groupCommands.forEach((command) => {
          ...
          table.addRow([
            chalk6.green(command.getName()),
            aliases ? chalk6.yellow(aliases) : "",
            shortcuts ? chalk6.magenta(shortcuts) : "",
...
var MakeCommand = class extends BaseCommand {
  ...
  async execute() {
    try {
      ...
      if (success) {
        this.success(
          `Command ${chalk6.green(this.commandName)} created successfully at ${chalk6.cyan(outputPath)}`
        );
        return BaseCommand.SUCCESS;
      } else {
...
var StubGenerator = class {
  ...
  generate(stubName, outputPath, replacements) {
    try {
      const stubPath = path2.join(this.stubsDir, `${stubName}.stub`);
      if (!fs2.existsSync(stubPath)) {
        throw new Error(`Stub template "${stubName}" not found.`);
      }
      let content = fs2.readFileSync(stubPath, "utf8");
...
var CommandCollector = class {
  ...
  static async discoverCommands(directory, pattern = "**/*-command.{ts,js}") {
    try {
      const absolutePath = path2.isAbsolute(directory) ? directory : path2.join(process.cwd(), directory);
...
export { ARGUMENT_METADATA_KEY, Application, Argument, Ask, BaseCommand, COMMAND_METADATA_KEY, Command, CommandCollector, CommandRegistry, DemoCommand, GreetCommand, HelpCommand, INJECTABLE_METADATA_KEY, Injectable, Input, ListCommand, MakeCommand, OPTION_METADATA_KEY, Option, Output, ProgressBar, ScheduleCommand, StubGenerator, TableOutput, container };
