...
var Output = class {
   ...
   */
  error(message) {
    console.error(chalk4.bold.red("ERROR") + ": " + message);
  }
  /**
   ...
   */
  success(message) {
    console.log(chalk4.bold.green("SUCCESS") + ": " + message);
  }
  /**
   ...
   */
  info(message) {
    console.log(chalk4.bold.blue("INFO") + ": " + message);
  }
  /**
   ...
   */
  warning(message) {
    console.log(chalk4.bold.yellow("WARNING") + ": " + message);
  }
  /**
...
var Application = class {
  ...
  registerWithCommander(command) {
    ...
    const commanderCommand = new Command$1(command.getName()).description(command.getDescription()).action(async (...args) => {
      try {
        ...
        console.error(
          chalk4.red(`Error: ${error instanceof Error ? error.message : String(error)}`)
        );
        process.exit(1);
      }
    });
  ...
  async run(argv = process.argv) {
    try {
      ...
      console.error(
        chalk4.red(`Fatal error: ${error instanceof Error ? error.message : String(error)}`)
      );
      process.exit(1);
    }
  }
...
var ProgressBar = class {
  ...
  constructor(total = 100, format) {
    this.bar = new cliProgress.SingleBar({
      format: (format == null ? void 0 : format.format) || `${chalk4.cyan("{bar}")} {percentage}% | ETA: {eta}s | {value}/{total}`,
  ...
  static createMultiBar() {
    return new cliProgress.MultiBar({
      format: `${chalk4.cyan("{bar}")} {percentage}% | ETA: {eta}s | {value}/{total}`,
      barCompleteChar: "\u2588",
      barIncompleteChar: "\u2591"
    });
  }
...
var DemoCommand = class extends BaseCommand {
  ...
  async demoAsk() {
    ...
    this.line();
    this.line(`Name: ${chalk4.cyan(name)}`);
    this.line(`Age: ${chalk4.cyan(age)}`);
    this.line(`Likes CLI: ${likesCli ? chalk4.green("Yes") : chalk4.red("No")}`);
    this.line(`Favorite color: ${chalk4.cyan(favoriteColor)}`);
    this.line(`Languages: ${languages.map((l) => chalk4.cyan(l)).join(", ")}`);
    this.line();
  }
  /**
...
var GreetCommand = class extends BaseCommand {
  ...
  async execute() {
    try {
      ...
      switch (color) {
        case "red":
          coloredGreeting = chalk4.red(greeting);
          break;
        case "green":
          coloredGreeting = chalk4.green(greeting);
          break;
        case "blue":
          coloredGreeting = chalk4.blue(greeting);
          break;
        case "yellow":
          coloredGreeting = chalk4.yellow(greeting);
          break;
        case "cyan":
          coloredGreeting = chalk4.cyan(greeting);
          break;
        default:
          coloredGreeting = chalk4.green(greeting);
      }
...
var ListCommand = class extends BaseCommand {
  ...
  async execute() {
    try {
      ...
      this.line(chalk4.bold("Available commands:"));
      this.line("");
      if (commands.length === 0) {
      ...
      for (const [category, groupCommands] of Object.entries(commandGroups)) {
        this.line(chalk4.cyan(`${category.charAt(0).toUpperCase() + category.slice(1)} Commands:`));
        ...
        groupCommands.forEach((command) => {
          ...
          table.addRow([chalk4.green(command.getName()), aliases ? chalk4.yellow(aliases) : "", command.getDescription()]);
        });
        table.render();
        this.line("");
      }
...
var MakeCommand = class extends BaseCommand {
  ...
  async execute() {
    try {
      ...
      if (success) {
        this.success(
          `Command ${chalk4.green(this.commandName)} created successfully at ${chalk4.cyan(outputPath)}`
        );
        return BaseCommand.SUCCESS;
      } else {
